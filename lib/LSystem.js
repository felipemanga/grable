CLAZZ("lib.LSystem", {
    ...
    source:function( src, _src ){
        ...
        /\s*#(lib|log|debug)>\s*([a-zA-Z0-9,]+)(?:\n\n|$)/g.forEach( src, ( match ) => {
            var type = match[1].toLowerCase();
            switch(type){
            case "lib":
                this.source( this.lib[ match[2] ] );
                break;

            case "log":
            case "debug":
                this[type] = ["on", "true"].indexOf(match[2].toLowerCase()) != -1;
                break;
            }
        });
