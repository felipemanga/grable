...
(function() {
    ...
    function getURL(url, cb, cfg) {
        ...
        if( /^blob:.*/.test(location.href) && !/^[a-z]+:.*/i.test(url) )
            url = location.origin + (DOC.locationPath || "/") + url;
            ...
            if ( obj.methods || clazz.methods ) {
                ...
                for (var i = 0; i < l; ++i) {
                    ...
                    if (m && m[0] != "_"){
                        ...
                        if( clazz.meta[m] && clazz.meta[m].silence )
                            this.silence(m);
                    }
                }
            } else {
    ...
    var DOC = {
        ...
        auto: function(set, cfg) {
            ...
            for (i = 0; i < l; ++i) {
                ...
                for (j = 0; j < desc.elements.length; ++j) {
                    ...
                    if (typeof desc.controller.update == "function") desc.controller.update(desc.value);
                }
            }
        ...
        app: function(path, cfg) {
            ...
            var file = path.replace(/\./g, '/') + ext, event = cfg.event || "DOMContentLoaded";
            document.addEventListener(event, function() {
                ...
                if (path == "") {
                    ...
                    if (ext == ".wire") {
                        ...
                        src += [
                            "var __queue=0, __onReadyCB = [];",
                            "var singleton = function(a, b){ __queue++; need(b, __cb); return singleton; },",
                            "\tmulti = singleton,",
                            "\timplements = singleton,",
                            "\tfactory = singleton,",
                            "\tmethod = function(a, b){ __queue++; need(b, __cb ); return method; },",
                            "\tjson = function(a, b){ __queue++; need([{URL:b.replace(/\\./g, '/') + '.json'}], function(){ ",
                                "\t\tif(a) DOC.resolve(a, null, DOC.resolve(b));",
                                "\t\t__cb();",
                            "\t} ); return json; },\n",
                            "\tset = function(){},",
                            "\tget = function(){},",
                            "\tonready = function( cb ){ __onReadyCB.push(cb); },",
                            "\toninit = function( cb ){ cb(); };",
                            wire,
                            "\n\nfunction __cb(){",
                            "__queue--;",
                            "if( __queue ) return;",
                            "var singleton = CLAZZ.singleton,",
                            "\timplements = CLAZZ.implements,",
                            "\tmulti = CLAZZ.multi,\n",
                            "\tfactory = CLAZZ.factory,",
                            "\tmethod = function(a, b){ if(a){ CLAZZ.set(a, self[a] = DOC.resolve(b)); } return method; },",
                            "\tjson = method,",
                            "\tset = CLAZZ.set,",
                            "\tget = CLAZZ.get,",
                            "\tonready = function(){},",
                            "\toninit = function(){};",
                            "__onReadyCB.forEach(cb => cb());",
                            wire,
                            "\n}\n",
                            "})();"
                        ].join("\n");
