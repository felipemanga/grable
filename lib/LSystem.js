            ...
            for( var i=0; i<arr.length; ++i ){
                ...
                if( arri.charAt ){
                    ...
                    if( s.length > 1 ){
                        ...
                        arr.splice.apply(arr, [i, 1].concat(s));
                    }
                }else if( arri.params ){
    ...
    _generate:function(matrix, str, depth){
        ...
        if( depth > 1 ){
            for( i=0; i<str.length; ++i ){
                stri = str[i];
                if( stri.charAt ) continue;
                var ret = this._generate(matrix, stri, depth-1);
                str.splice.apply(str, [i, 1].concat( ret ) );
                i += ret.length;
            }
        }
        ...
        next: for( var j=0; j<exp.length; ++j ){
            for( var k=0; k<index.length; ++k ){
                ...
                if( str[strpos] == "(" ){
                    ...
                    var pc = 1, acc="";
                    do{
                        if(strpos>=str.length) throw new Error("Unmatched Parenthesis");
                        var ch = str[strpos++];
                        if( ch == ")" ) pc--;
                        else if( ch == "(" ) pc++;
                        if( (ch == "," && pc == 1) || (ch == ")" && pc == 0) ){
                ...
                if( strpos < str.length ) out[out.length] = str.substr(strpos);
