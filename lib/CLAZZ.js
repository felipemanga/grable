...
(function(){
    ...
    CLAZZ = function( name, clazz ){
        ...
        var ret = function(){
            ...
            if( this == self )
                return new (ret.bind.apply(ret, [null].concat( args )));
...
/* */
    		...
    		if( ctor )
    		{
				...
				if( !unserializing ) ctor.apply( this, args );
    		...
    		for( var k in cclazz )
    		{
    			...
    			if( k == "MIXIN" ){
    			    for( sk=0; sk<v.length; ++sk ){
                        ...
                        if(mix.methods)
                            addMerge(methods, mix.methods);
                        if(mix.properties)
                            addMerge(props, mix.properties);
    			    }
    			    continue;
    			}
    			if( k == "STATIC" ){
    ...
    CLAZZ.get = function( m, context, THIS, def ){
    	...
    	}else if( typeof m == "function" ) bind = () => new m();

		if( !bind ){
			var ctxId = dibindings.length-1;
			do{
